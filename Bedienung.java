...
public class Bedienung extends JFrame {
	...
	private class OutputWorker extends SwingWorker<String, String> {
	       protected String doInBackground() throws Exception
	       {
	           // Do a time-consuming task.
	           Thread.sleep(2000);
	           String myAnswer = downloadUrl("http://192.168.0.111:821/getconsumption?typ=wasser");
	           return myAnswer;
	       }
	       protected void done()
	       {
	           try
	           {
	        	   if(wass) wassfield.setText(get());
	        	   if(gase) gasfield.setText(get());
	        	   if(stroeme) stromfield.setText(get());
        ...
        try {
            ...
            if (contentAsString.contains("wasser")) {
            	int w = contentAsString.indexOf("wasser=\"");
            	String wa = contentAsString.substring(w);
            	wass = true;
            	return wa; 
            }
            if (contentAsString.contains("strom")) {
            	int s = contentAsString.indexOf("strom=\""); 
            	String st = contentAsString.substring(s);
            	stroeme = true;
            	return st;
            }
            if (contentAsString.contains("gas")) {
            	int g = contentAsString.indexOf("gas=\"");
            	String ga = contentAsString.substring(g);
            	gase = true;
            	return ga; 
            }
            else {
 ...
 public void writeIt(String value, HttpURLConnection conn) throws IOException, UnsupportedEncodingException {
     Writer writer = null;
     writer = new PrintWriter(conn.getOutputStream());
     writer.write(value);
 } 
	private class AnswerWorkerSet extends SwingWorker<String, String> {
	       protected String doInBackground() throws Exception
	       {
	           // Do a time-consuming task.
	           Thread.sleep(2000);
	           String myAnswer = downloadUrl("http://192.168.0.111:821/consumption?typ=wasser&date=20160329&value=222.99");
	           return myAnswer;
	       }
	       protected void done()
	       {
	...
	}private void downloadUrl1(String myurl) throws IOException {
  InputStream is = null;
  OutputStream os = null;
  // Only display the first 500 characters of the retrieved
  // web page content.
  ...
  try {
      ...
      writeIt(getWassfield().getText(), conn);
